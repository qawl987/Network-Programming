...
struct Command {
    vector<string> args;
    array<int, 2> pipe = {STDIN_FILENO,
                          STDOUT_FILENO}; // 0: fd_in, 1: fd write to fd_in
...
vector<Command> parseCommands(const string &line,
                ...
                if (pipeMap.count(pipeNum) == 0) { // create pipe
                    ...
                    while (pipe(pipe_fd.data()) == -1) {
                        if (errno == EMFILE || errno == ENFILE) {
                            wait(nullptr); // wait for any child process to
                                           // release resource
                        }
                    }
...
int main() {
    ...
    while (true) {
        ...
        vector<Command> commands = parseCommands(line, pipeMap);
